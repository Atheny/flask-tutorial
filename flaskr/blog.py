...
from flask import Blueprint, g, redirect, render_template, flash, url_for, request
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'UPDATE post SET title = ?, body = ? WHERE id = ?', (title, body, post['id'])
            )
            db.commit()
            return redirect(url_for('blog.index'))
